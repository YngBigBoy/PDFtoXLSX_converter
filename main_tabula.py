...
class PDFtoExcelConverter(QWidget):
            ...
            if destination_folder:
                for pdf_file in files:
                    try:
                        ...
                        #QMessageBox.information(self, "Успех", f"Конвертация {os.path.basename(pdf_file)} завершена успешно!")
                ...
                if not(exept):
                    QMessageBox.information(self, "Успех",
                                        f"Конвертация PDF-файла(ов) завершена успешно!")
        ...
        if files:
            ...
            if save_path:
                try:
                    ...
                    QMessageBox.information(self, "Успех",
                                            f"Конвертация PDF в {os.path.basename(save_path)} завершена успешно!")
                    gc.collect()
    ...
    def convert_pdf_to_excel(self, pdf_path, destination_folder):
        ...
        for table in tables:
            ...
            current_header = list(table.columns)
            table_df = pd.DataFrame(table)
            # table_df.insert(0, 'Файл', os.path.basename(pdf_path))  # Добавляем название файла в первый столбец
            ...
            for r in dataframe_to_rows(table_df, index=False,
                                       header=(start_row == 1 or (previous_header != current_header))):
                for c_idx, value in enumerate(r, 1):
                    cell = ws.cell(row=start_row, column=c_idx, value=value)

                    # Применяем форматирование
                    if start_row == 1 or (r == list(table_df.columns)):
        ...
        for pdf_file in pdf_files:
            ...
            for table in tables:
                if table.empty or table.dropna(how='all').empty:
                    continue
                table_df = pd.DataFrame(table)
                table_df.insert(0, pdf_filename, pdf_filename)
                # Проверяем заголовок текущей таблицы
                if list(table_df.columns) != current_header:
                    if current_header != list(table_df.columns):
                        start_row += 3  # отступаем на 3 строки между разными таблицами из одного файла
                ...
                # Записываем данные таблицы
                for r in dataframe_to_rows(table_df, index=False,
                                           header=(start_row == 1 or (current_header != list(table_df.columns)))):
                    for c_idx, value in enumerate(r, 1):
                        cell = ws.cell(row=start_row, column=c_idx, value=value)
                        ...
                        if start_row == 1 or (r == list(table_df.columns)):
                            ...
                            cell.alignment = Alignment(horizontal='left', vertical='center')
                    start_row += 1
                current_header = list(table_df.columns)
