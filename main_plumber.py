...
class PDFtoExcelConverter(QWidget):
            ...
            if destination_folder:
                for pdf_file in files:
                    try:
                        ...
                        gc.collect()
                QMessageBox.information(self, "Успех", "Конвертация PDF-файлов завершена успешно!")
    ...
    def convert_pdf_to_excel(self, pdf_path, destination_folder):
        ...
        for table_df in tables:
            ...
            for r in dataframe_to_rows(table_df, index=False, header=(start_row == 1 or (previous_header != current_header))):
    ...
    def convert_pdfs_to_one_excel(self, pdf_files, excel_path):
        ...
        for pdf_file in pdf_files:
            ...
            for table_df in tables:
                ...
                # Записываем данные таблицы
                for r in dataframe_to_rows(table_df, index=False, header=(start_row == 1 or (current_header != list(table_df.columns)))):
                    for c_idx, value in enumerate(r, 1):
                        cell = ws.cell(row=start_row, column=c_idx, value=value)
